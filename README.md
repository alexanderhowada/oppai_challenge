...
Many companies, including the company that I currently work at (WMcCann, a 5 billion USD worth agency),
run their data pipelines on databricks deployments like this one.
check out [Alexander Wada's GitHub](https://github.com/alexanderhowada/oppai_challenge.git) for the source code.

# Features (data analyst/engineering/science):
- Deploy on Databricks AWS
    - Lakehouse (data lake + warehouse). See [Databricks Blog](https://www.databricks.com/blog/2020/01/30/what-is-a-data-lakehouse.html) for details.
    - Integration with AWS S3 storage and other services.
    - Fully scalable.
- ACL
    - While I am not an expert in ACL, Databricks OFFERS ITS OWN ACL SOLUTION FOR ITS RESOURCES.
...
- Dashboards
    ...
    - Can be deployed as custom dashboards (costs a minimum of 1 USD per hour!)
- Machine learning (see machine_learning folder)
    - Integration with OpenAI
    - Simple regression example (can be upgraded to a forecaster)

# Infrastructure

Databricks is a platform that comprises functionalities for data engineers, analysts, and scientists.
...
uses their infrastructure to run.
Therefore, the data is stored within the cloud provider and not on Databricks!

For pricing, we have to pay for the cloud provider resources + Databricks.
The total pricing for this deployment is approximately:
2.5 USD per day (1.5 for AWS and 1.0 for Databricks)
plus a few cents for the ChatGPT API.
Here brief pricing breakdown of this deployment:
...
Despite its high price, it is important to remember that Databricks is an all-in-one solution.
The same solution without Databricks and with similar functionalities should use/implement:
...
2. Dashboard solution (such as Power BI or Tableau)
...
5. Workspace
    - Fully collaborative workspace that avoids problems with local development

Also, notice that Databricks is also capable of deploying Machine Learning solutions,
and that companies such as <b>iFood</b> USE IT.

# S3 integration and lakehouse.

Databricks allows mounting S3 in its file system.
...
in order to give the appropriate permissions in AWS.
...
for more details.
Delta tables are highly compressed and stored in S3, hence they are much cheaper than
...
with the addition of the raw layer:
...
Since they are views, we ensure that the latest data is always there.
...
their "serverless" solution always runs two EC2 VMs that cost about 1 USD per hour,
...
Some of these notebooks feature widgets at the top whose values can be changed to customize the view.
After changing the values, please run the cell again to update the results.
...
These notebooks contain a few annotations explaining a few of the results.
...
Here I would like to highlight the integration with ChatGPT
for <b>summarizing thousands of post comments in a few seconds</b>.
While it is interesting to implement my own solution,
it is impossible to compete with the state-of-the-art developments of OpenAI.
...
Instead, I preferred to provide a full ETL pipeline with quality and unittests,
...
There is no need to complicate things that can be simple.
Here is a breakdown of things that I considered:
- dataset size:  
    > small datasets do not need a lot of maintenance, therefore there is no need to complicate it.
- easy maintenance: 
...
- separate translations from posts:
    ...
    Therefore, separating the posts from its translations seems a right business decision.
...
- No dimensional modeling:
    > I decided to not use dimensional modeling due to the small size of the dataset; however, I would implement it if the dataset were larger (terabytes).

- ETL by updated_at_date:
    > for scalability purposes, it is interesting to process only the data that have changed. Therefore, I process data in accordance with its updated date.

    > processing by updated_at_date is a flexible process as the same script can be used to process the entire dataset and or the latest data.

## Essa modelagem serviria pra um treinamento de um modelo de machine learning?

Machine learning pipelines can be very complex and may require very specific data transformations and enrichment.
...
1. We want to build a model that classifies post comments as relevent/irrelevant suggestions. In this case we could use the table built at step 1 and create a new ML table with new features for the comments. Among these features, we could include number of words, number of complex words, metrics for [text readability](https://en.wikipedia.org/wiki/Readability), and [lexical diversity](https://en.wikipedia.org/wiki/Lexical_density). With the two new tables, we could build a model to select the relevant suggestions and forward them to the staff.
1. We want to build a simple forecaster to predict the monetary contribution at the end of 2024. By keeping historical data (raw tables), we could build a new (bronze) table with timeseries of monetary contribution. This new table could be used to build a ML model to create a forecaster. The results of the forecaster could be used to compare our yearly growth.
...
however, the latter format is a lot clearer and simplifies the ETL a lot.

Why the second format is better?
The second format is better since it simplifies column names and eliminates the possibility of new nested data
```
{
...
- Add README, typing and docstrings
    ...
    > typing plays a very important role in Python as it makes it simple to understand the inputs of functions and methods and is very important when creating reusable code.

    > similarly to typing, docstrings is also very important when writing reusable code.
- Better governance
    > Governance is very important and writing a document that describes the properties or raw, bronze, silver and gold tables and their columns is very important.
    For example, I could set that no column in the silver and gold tiers has null values. This would improve the data analysis because it avoids the use of the COALESCE statement.
- Better data quality
...
- Better unit/integration tests
    > Unittests ensure that the code I wrote does what I want it to do, and writing them is an investment for the future. It only takes about 3 to 6 months until it becomes extremaly important.
- CI/CD
...
- Implement unity catalog
    > Unity catalog allows ACL for the delta tables. This is very important when working with many collaborators, but it requires bigger machines and therefore an increase in price.
### Data science
...
In addition, I decided that it was more important to integrate ChatGPT than to implement an ML model.
...
To improve, I need to start from the very basics and take lessons on history telling and understand more about the business.
I also could have made a detailed exploratory data analysis, but I decided to do just the necessary and focus on deployment.

I believe that I could also have generated a report with all the results.
This would have made it a lot easier to understand the results.
